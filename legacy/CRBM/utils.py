...
try:
    from . import config
except ImportError:  # Keeps legacy scripts that import CRBM modules directly working.
    import config
...
def process_array_to_pytorch(dataset, w_shape, p_shape ):
    temp=[]
    for i in range(len(dataset)):
        temp.append( trim_array_maxpool(arr=dataset[i,:,:], conv_window_shape=w_shape, pooling_shape=p_shape) )
    img_array = np.array(temp)
    return torch.from_numpy(img_array[:,np.newaxis,:,:]).float()    
...
def trim_array_maxpool(arr=None, arr_shape=None, conv_window_shape=0, pooling_shape=0):
    ...
    t_trim = int(math.floor(h_trim/2.0))
    b_trim = int(math.ceil(h_trim/2.0))
    
    assert t_trim + b_trim == h_trim
    
    w_trim = (w-wc+1) % pooling_shape[1]
    l_trim = int(math.floor(w_trim/2.0))
    r_trim = int(math.ceil(w_trim/2.0))
